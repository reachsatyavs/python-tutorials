...
List / dict / set comprehensions and generators are the same *idea* (iterate, optionally filter, produce values) with two flavours: **eager** (materialise a whole `list`, `dict`, or `set`) vs **lazy** (generator expression or `yield` function). 
