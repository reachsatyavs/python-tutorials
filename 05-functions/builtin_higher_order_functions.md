# `sorted`, `min`, `max`, `map`, `filter`, `reduce` — Regular Function vs Lambda

> All examples use the same employee list.  
> Every example is shown **twice** — once with a named function, once with a lambda — so students can see that both do exactly the same thing.
...
```python
maxfrom functools import reduce
